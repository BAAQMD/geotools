...
ggplot_map_RSP <- function (
  ...
  if (isFALSE(is.null(tileset))) {
    ...
    if (tileset == "toner") {
      tileset <- "http://tile.stamen.com/toner/{z}/{x}/{y}.png"
      tile_opacity <- (tile_opacity %||% 0.3) # default to 0.3
      coastline <- (coastline %||% FALSE)
      zoom <- (zoom %||% 11)
    }
    ...
    if (librarian:::is_valid_url(tileset)) {
      ...
      msg("invoking ggplot_tileset(url = '", tileset, "', ...)")
      map_object <-
        ggplot_tileset(
          url = tileset,
          zoom = zoom,
      ...
      if (str_detect(tileset, "terrain|roadmap|hybrid")) {
  ...
  if (isFALSE(is.null(tileset))) {
    ...
    tile_opacity <- (tile_opacity %||% 0.3)
    msg("tile_opacity is: ", tile_opacity)

    buffered_envelope <-
      st_buffer(st_transform(envelope, WEB_MERCATOR), 10e3)

    tile_opacity_layer <-
      geom_sf(
        fill = "white",
        color = NA,
        alpha = I(1 - tile_opacity),
        inherit.aes = FALSE,
  ...
  if (isTRUE(coastline)) {
    ...
    coastline_stencil <-
      ...
      st_intersection(st_buffer(envelope, dist = 60e3)) %>%
      st_transform(crs)
  ...
  attr(map_object, "map_annotations") <-
    map_annotations
